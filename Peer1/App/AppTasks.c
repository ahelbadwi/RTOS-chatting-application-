...
ErrorStatus_t UART_RxcompleteCBK(void)
{
	...
	if(pdTRUE == xQueueSendFromISR(ReceivedUartKeypadMsg_Queue,&au8_receivedKeypadMsg[ByteCounter],NULL))
	{
		if(au8_receivedKeypadMsg[ByteCounter] != NULL_CHAR)
		{
			ByteCounter++;
		}
		else
...
void KeyPad_Task(void *pvParameters)
{
	...
	while(1)
	{
		...
		Keypad_cyclic_Getbutton((uint8*)&au8_Btn[ZERO_VALUE],(Keypad_state_t*)&Keypad_state);
	...
	while(1)
	{
		...
		if(TRUE == b_LocalPressedFlag)
		{
			...
			if(KeypadBtnCounter2 == KeypadBtnCounter1)
			{
				...
				xSemaphoreGive(Uart_BinarySemaphore);
...
void Uart_SendTask(void *pvParameter)
{
	...
	static uint8 au8_receivedKeypadMsg[MAX_KEYPAD_MESSAGE];
	const TickType_t xTask_periodicity=10;
	/* The xLastWakeTime variable needs to be initialized with the current tick count */
	TickType_t xLastWakeTime=xTaskGetTickCount();
	while(1)
	{
...
{
	extern QueueHandle_t ReceivedUartKeypadMsg_Queue;
	extern SemaphoreHandle_t RxLed_Semaphore;

	static uint8 u8_receivedMsgCounter1=ZERO_VALUE;
	static uint8 au8_receivedKeypadMsg[MAX_KEYPAD_MESSAGE];
	const TickType_t xTask_periodicity=UART_RX_TASK_PERIODICITY;
	/* The xLastWakeTime variable needs to be initialized with the current tick count */
	TickType_t xLastWakeTime=xTaskGetTickCount();
	while(1)
	{
		if( RxLed_Semaphore != NULL )
		{
			/* Take Led semaphore */
			xSemaphoreTake( RxLed_Semaphore,0 );
		}

		/* Receive keypad message once the push button is pressed */
		if(pdPASS == xQueueReceive(ReceivedUartKeypadMsg_Queue,&au8_receivedKeypadMsg[u8_receivedMsgCounter1],0))
		{

			/* check if the message end */
			if(NULL_CHAR ==au8_receivedKeypadMsg[u8_receivedMsgCounter1])
			{
				/* Give semaphore(signal) to the Led Toggling task as an indication of receiving new message */
				xSemaphoreGive( RxLed_Semaphore );
				/* Resetting the counter for the next message */
				u8_receivedMsgCounter1=ZERO_VALUE;
				/* Display the received message On Row2 of the LCD */
				LCD_Clear_ROW(SECOND_ROW);
				LCD_Display_Request((char*)au8_receivedKeypadMsg);
				/* Block task for 2 seconds */
				vTaskDelay(2000);
				LCD_Clear_ROW(SECOND_ROW);
				/* go to home of LCD to transmit new message */
				LCD_gotoxy(1,1);
			}
			else
			{
				/* Increment received counter to save the next received byte in au8_receivedKeypadMsg buffer */
				u8_receivedMsgCounter1++;
			}
		}
		else
 ...
 * 		-				NA
 *******************************************************************************/
void LedToggle_400(void *pvParameters)
{
	extern SemaphoreHandle_t RxLed_Semaphore;
	...
	while(1)
	{
		if( RxLed_Semaphore != NULL )
		{
			if(xSemaphoreTake( RxLed_Semaphore,0 )==pdTRUE)
			{
				Led_On(LED_1);
				vTaskDelay(400);
				Led_Off(LED_1);
